...
class ChartData(CamelizedBaseModel):
    ...
    value: str | float | int = Field(description='значение данных, если это текстовые данные, то это представляет из себя один абзац полностью развёрнутого ответа, который включает в себя так же вопрос')
...
class Widget(CamelizedBaseModel):
    ...
    category: list[str] = Field(
        description='необязательное поле, которое может содержать данные о том, как лучше разбивать данные, обычно пустая'
    )
